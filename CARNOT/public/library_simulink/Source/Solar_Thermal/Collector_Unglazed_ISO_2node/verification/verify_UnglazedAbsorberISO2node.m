%% Verification of the model Collector_Unglazed_ISO_2node
%% Function Call
%  [v, s] = verify_UnglazedAbsorberISO2node(show, safe_simref)
%% Inputs
...
%  Function uses: verify_UnglazedAbsorberISO2node_mdl.slx
%  See also template_verify_mFunction, template_verify_SimulinkBlock, verification_carnot

function [v, s] = verify_UnglazedAbsorberISO2node(varargin)
%% Calculations
...
elseif nargin == 2
    ...
    error('verify_UnglazedAbsorberISO2node:%s',' too many input arguments')
end
...
functionname = 'verify_UnglazedAbsorberISO2node_mdl';
t0 = 1800:900:14*3600;
...
y0 = yt.data(:,2)*1e-3;         % reference power values from fcn block (iso equation)
y2 = yt.data(:,3)*1e-3;         % power from model
y2(:,2) = yt.data(:,4);         % pressure drop from model
y0(:,2) = ones(size(u0))*200.276; % reference pressure drop from TUEV test report
close_system(functionname, 0)   % close system, but do not save it

%% set reference values initial simulation
if (save_sim_ref)
    y1 = y2;   % determinded data
    save('simRef_CollectorUnglazedISO2node.mat','y1');
else
    y1 = importdata('simRef_CollectorUnglazedISO2node.mat');  % result from call at creation of function
end
...
if (show)
    ...
    sy1 = 'Power in kW';                     % y-axis label in the upper plot
...
%  7.1.0    drhf    created on the base of                      14mar2021
%                   verify_CollectorUnglazedISO9806
% * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
