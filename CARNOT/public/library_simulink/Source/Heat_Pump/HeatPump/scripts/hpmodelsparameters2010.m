...
% pressure drop source side estimated values: 
dP_source =[16.1480954076184e+003    2.37735849056604e+003;
    25.0000000000000e+003    5.00000000000000e+003; ...
    25.1033057851240e+003    6.13636363636363e+003; ...
    17.9733727810651e+003    5.19230769230769e+003; ...
    13.4201183431953e+003    4.84615384615384e+003];
...
hptext = char('BW106','BW108','BW110','BW113','BW117');

for hptype = 1:length(Ch)
    HP.caph = Ch(hptype); 
    HP.capc = Cc(hptype); 
    HP.K = Kbw(hptype,:);
    linh = dP_sink(hptype,2);
    quah = dP_sink(hptype,1);
    linc = dP_source(hptype,2);
    quac = dP_source(hptype,1);
    HP.UA = UAbw(hptype);
    s = ['save(''Vitocal300G_',hptext(hptype,:),''', ''HP'')'];
    eval(s);
end
